...
export default function NightPhase({ players, phase, onAction, checkedPlayer }: NightPhaseProps) {
  const getPhaseInfo = () => {
    switch (phase) {
      case 'NIGHT_MAFIA':
        return { title: 'Ход Охотников', desc: 'Выберите цель для устранения', icon: Target, color: 'text-rose-mafia' };
      case 'NIGHT_DETECTIVE':
        return { title: 'Ход Альфы', desc: 'Выберите игрока для проверки', icon: Eye, color: 'text-blue-400' };
      case 'NIGHT_DOCTOR':
        return { title: 'Ход Целителя', desc: 'Выберите, кого защитить', icon: Shield, color: 'text-green-400' };
      default:
      ...
      {checkedPlayer && (
        <div className="glass-card p-4 mb-6 text-center border-blue-400/30 shrink-0">
          ...
          <p className="text-sm font-bold text-blue-400">
            {players.find(p => p.id === checkedPlayer.id)?.name} — {checkedPlayer.role === 'Mafia' ? 'ОХОТНИК' : 'ДРАКОН'}
          </p>
        </div>
      )}
