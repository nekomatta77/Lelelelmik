...
export default function GameSetup({ onStart, tgUser }: SetupProps) {
  ...
  // ==========================================
  // ⚙️ НАСТРОЙКИ ДИЗАЙНА (МЕНЯЙ ЦИФРЫ ЗДЕСЬ)
  // ==========================================
  const DRAGON_IMG = {
    scale: 2.4,  // Размер дракона (1 = обычный, 1.5 = на 50% больше, 0.8 = меньше)
    x: 0,        // Движение дракона влево/вправо (отрицательные числа - влево, положительные - вправо)
    y: 80        // Движение дракона вверх/вниз (отрицательные - вверх, положительные - вниз)
  };

  const UI_BLOCK = {
    x: 0,        // Движение всего интерфейса (текст + лобби + кнопка) влево/вправо
    y: 200        // Движение всего интерфейса вверх/вниз
  };
  // ==========================================
  ...
  return (
    ...
    >
      ...
      {/* ВЕСЬ ИНТЕРФЕЙС, КОТОРЫЙ МОЖНО ДВИГАТЬ РАЗОМ */}
      <motion.div 
        className="w-full flex flex-col items-center"
        initial={{ y: UI_BLOCK.y, x: UI_BLOCK.x }}
        animate={{ y: UI_BLOCK.y, x: UI_BLOCK.x }}
      >
