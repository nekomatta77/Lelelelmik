    ...
    >
      <div className="text-center mb-6 shrink-0">
        ...
        <h2 className="text-2xl font-bold uppercase">
          {isVoting ? 'Голосование' : 'Наступило утро'}
        </h2>
      </div>
      ...
      {!isVoting && (
        <div className="glass-card p-6 mb-6 text-center border-white/10 shrink-0">
          {victim ? (
            <>
              <p className="text-white/40 text-[10px] uppercase mb-2">Этой ночью стая потеряла:</p>
              <p className="text-xl font-black text-rose-mafia">{victim.name}</p>
            </>
          ) : (
            <p className="text-sm font-bold text-green-400 uppercase">Ночь прошла спокойно. Никто не погиб.</p>
          )}
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 mb-6">
        {isVoting ? (
          <div className="space-y-3">
            <p className="text-center text-[10px] text-white/30 uppercase mb-4">Выберите, кого изгнать из стаи:</p>
          ...
          <div className="text-center space-y-6 flex flex-col items-center justify-center h-full pb-10">
            ...
            <p className="text-sm text-purple-100/60 leading-relaxed italic px-4">
              Обсудите события ночи. Кто ведет себя подозрительно? У вас есть время до начала голосования.
            </p>
          </div>
        )}
      </div>
